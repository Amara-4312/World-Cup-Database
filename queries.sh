...
echo  -e "$($PSQL "SELECT name FROM teams JOIN games ON teams.team_id = games.winner_id or teams.team_id = games.opponent_id WHERE year =2014 AND round = 'Eighth-Final' ORDER BY name")"
...
echo -e "$($PSQL "SELECT games.year, teams.name FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE round = 'Final' ORDER BY year")"
